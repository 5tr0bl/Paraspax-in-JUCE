...
def process_and_plot_wav(wav_file_path, plot_path, plot_type):
    ...
    if (plot_type == 'mag'):
        ...
        ax = pf.plot.time(transfer_function[1])
        ax.set_xlim([0.01, 0.02])
        plt.close()
        fig = ax.get_figure() # Get the figure from the existing axes
        fig.suptitle(file_path + ' Right', fontsize=16)
        fig.savefig(plot_file_name, bbox_inches='tight')
...
measurement_paths = []
measurement_paths.append("Python\Measurements\Specular BRIRs")

plots_paths = []
plots_paths.append("Python\Plots\Specular BRIRs")

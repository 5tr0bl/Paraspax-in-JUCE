...
)
{
    ...
    *phBin = (void*)pData;
    ...
    pData->nSources = NUM_DELAY_PROCESSORS; // originally = pData->new_nSources;
    pData->new_nSources = NUM_DELAY_PROCESSORS; // setting for safety reasons
    ...
    if ((nSamples == BINAURALISER_FRAME_SIZE) && (pData->hrtf_fb!=NULL) && (pData->codecStatus==CODEC_STATUS_INITIALISED) ) {
        ...
        for (ch = 0; ch < nSources; ch++) {
            ...
            if (pData->recalc_hrtf_interpFLAG[ch]) {
                ...
                pData->recalc_dvfCoeffFLAG[ch] = 1; // set this to 0 to avoid the DVF stuff...
            }
            /* Update DVF filters with change in direction and/or distance */
            /* Custom flag "DVF_PROCESSING_ENABLED" set manually in _common.h */
            /* (ch == 0) in order to do DVF processing only for direct source */
            if (/*DVF_PROCESSING_ENABLED && */(ch == 0) && pData->recalc_dvfCoeffFLAG[ch]) {
            ...
            if (/*DVF_PROCESSING_ENABLED && */(ch == 0) && pData->src_dists_m[ch] < ffThresh) {
...
{
    /* Is also the setter for src_gains[] */
    binauraliserNF_data *pData = (binauraliserNF_data*)(hBin);
    ...
    if(pData->src_dists_m[index] != newDist_m){
        ...
        // remove source gain adaptation here
        //binauraliser_setSourceGain(hBin, index, SAF_CLAMP(1.0f / newDist_m, 0.0f, 1.0f));
    }
}

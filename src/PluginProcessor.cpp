...
void PluginProcessor::processBlock(AudioBuffer<float>& buffer, MidiBuffer& /*midiMessages*/)
{
    ...
    if((nCurrentBlockSize % frameSize == 0)){ /* divisible by frame size */
        ...
        if (dfpEnabled && !idleLoadingConfig) {
            ...
            buffer.addFrom(0, 0, dfpOutputBuffer, 0, 0, nCurrentBlockSize);// , revAmplitude);
            buffer.addFrom(1, 0, dfpOutputBuffer, 1, 0, nCurrentBlockSize);// , revAmplitude);
        }
    }
    else
        buffer.clear();
...
void PluginProcessor::loadConfiguration (const File& configFile)
{
    ...
    if(result.wasOk()){
        ...
        updateRoomDims = true;
        updateXyPadPosition = true; // this will be checked by PluginEditor'stimer callback

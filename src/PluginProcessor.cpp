...
PluginProcessor::PluginProcessor() : 
	AudioProcessor(BusesProperties()
		.withInput("Input", AudioChannelSet::discreteChannels(64), true) // set to 1
    ...
    * so this here will be handling the VAE positioning parameters only
    * Part of the reasons is to create a state where the XY Pad does not dominate the Sliders and vice versa
    * This will also make it easier to be automated by a DAW
    */
    apvts(*this, nullptr, "positionParams", {
            // Parameter layout
            std::make_unique<AudioParameterFloat>("srcPosX", "Source Position X", 0.f, 80.f, 0.f),
            std::make_unique<AudioParameterFloat>("srcPosY", "Source Position Y", 0.f, 80.f, 0.f),
            std::make_unique<AudioParameterFloat>("srcPosZ", "Source Position Z", 0.f, 80.f, 0.f),
            std::make_unique<AudioParameterFloat>("lisPosX", "Listener Position X", 0.f, 80.f, 0.f),
            std::make_unique<AudioParameterFloat>("lisPosY", "Listener Position Y", 0.f, 80.f, 0.f),
            std::make_unique<AudioParameterFloat>("lisPosZ", "Listener Position Z", 0.f, 80.f, 0.f),
        })
{
...
const String PluginProcessor::getParameterName (int index)
{
    ...
    if(index < k_NumOfParameters){
        switch (index) {
            ...
            case k_lisPosX:         return "lis_pos_x";
            case k_lisPosY:         return "lis_pos_y";
            case k_lisPosZ:         return "lis_pos_z";
            default: return "NULL";
        }
    }
    /* source direction parameters */
...
void PluginProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    if(psxLoaded)       
        dfp.init(dfpSpec, mixingTime, sampleRate); // think of other things, that should only be done after import
}
...
void PluginProcessor::processBlock(AudioBuffer<float>& buffer, MidiBuffer& /*midiMessages*/)
{
    ...
    if((nCurrentBlockSize % frameSize == 0)){ /* divisible by frame size */
        ...
        if (dfpEnabled && !idleLoadingConfig) {
            ...
            buffer.addFrom(0, 0, dfpOutputBuffer, 0, 0, nCurrentBlockSize, diffusePartScalingFactor);
            buffer.addFrom(1, 0, dfpOutputBuffer, 1, 0, nCurrentBlockSize, diffusePartScalingFactor);
        }
    }
    else
        buffer.clear();
    ...
    if(result.wasOk()){
        ...
        for (ValueTree::Iterator it = sources.begin() ; it != sources.end(); ++it){
            if ( !((*it).getProperty("Imaginary"))){
                ...
                /*
                delayProcessors[src_idx].setAzimuth((*it).getProperty("Azimuth"));      This will happen in initDelayProcessors()
                delayProcessors[src_idx].setElevation((*it).getProperty("Elevation"));
                delayProcessors[src_idx].setDistance((*it).getProperty("Radius"));
                */
                src_idx++;
            }
        }
        ...
        updateRoomDims = true;
        updateXyPadPosition = true; // this will be checked by PluginEditor's timer callback
        ...
        idleLoadingConfig = false; // this flag is to prevent most of the processing in the PluginProcessor during config load
    }
}
...
void PluginProcessor::initDelayProcessors(juce::dsp::ProcessSpec& spec)
{
    ...
    for (int i = 0; i < NUM_DELAY_PROCESSORS; ++i) {
        ...
        //delayProcessors[i].setMaxDelayTime(MAX_DELAY, sampleRate);
        //delayProcessors[i].setDelayTimeInSamples(DEFAULT_DELAY_IN_SAMPLES, sampleRate); --> included in setDistance call

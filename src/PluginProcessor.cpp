...
void PluginProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    if(psxLoaded)       
        dfp.init(dfpSpec, mixingTime); // think of other things, that should only be done after import
}
...
void PluginProcessor::processBlock(AudioBuffer<float>& buffer, MidiBuffer& /*midiMessages*/)
{
    ...
    for (int inputChannelIndex = 0; inputChannelIndex < NUM_DELAY_PROCESSORS; ++inputChannelIndex) {
        // Copy input 0 to all inputs > 0
        if(inputChannelIndex > 1)
            buffer.copyFrom(inputChannelIndex, 0, buffer, 0, 0, nCurrentBlockSize);
    }
    // Fill the DiffuseFieldProcessor's internal buffer
    dfp.inputBuffer.setSize(2, nCurrentBlockSize, true, false, false);
    dfp.inputBuffer.copyFrom(0, 0, buffer, 0, 0, nCurrentBlockSize);
    dfp.inputBuffer.copyFrom(0, 0, buffer, 1, 0, nCurrentBlockSize);
    ...
    if((nCurrentBlockSize % frameSize == 0)){ /* divisible by frame size */
        ...
        if (dfpEnabled) {
            dfp.process();
            // Add the processed audio back to the first two channels of the buffer
            // buffer still has N=NUM_DELAY_PROCESSORS channels, but binaural mix is on channels 1&2 only
            buffer.addFrom(0, 0, dfp.inputBuffer, 0, 0, nCurrentBlockSize, revAmplitude);
            buffer.addFrom(1, 0, dfp.inputBuffer, 0, 0, nCurrentBlockSize, revAmplitude);
        }
    }
    else
        buffer.clear();
...
void PluginProcessor::loadConfiguration (const File& configFile)
{
    ...
    Result result = ParaspaxConfigurationHelper::parseFileForParaspaxVariables(configFile, sources, nullptr, startEndArray,
                                                                               rirBuffer, dfp.binRevBuffer, 
                                                                               dfp.inverseWeightingFunc, revAmplitude, mixingTime);
    ...
    if(result.wasOk()){
        ...
        for (ValueTree::Iterator it = sources.begin() ; it != sources.end(); ++it){
            if ( !((*it).getProperty("Imaginary"))){
                ...
                delayProcessors[src_idx].setAzimuth((*it).getProperty("Azimuth"));
                delayProcessors[src_idx].setElevation((*it).getProperty("Elevation"));
                delayProcessors[src_idx].setDistance((*it).getProperty("Radius")); // Hope this does not crash..
                src_idx++;
            }
        }
        refreshWindow = psxLoaded = true; // now the DelayProcessors can get their MIR excerpts
        idleLoadingConfig = false; // this flag is to prevent the DelayProcessors from processing during config load
    }
}
...
void PluginProcessor::initDelayProcessors(juce::dsp::ProcessSpec& spec)
{
    ...
    // Random values for filter cutoff
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dis(1000.0, 12000.0);
    ...
    for (int i = 0; i < NUM_DELAY_PROCESSORS; ++i) {
        ...
        //delayProcessors[i].setFirFilter(dis(gen), sampleRate);
        delayProcessors[i].setFirFilter(20000, sampleRate);
        delayProcessors[i].setDistance(bData->src_dists_m[i]);
        if (psxLoaded) {

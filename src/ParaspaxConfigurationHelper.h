...
public:
    ...
    static Result parseFileForParaspaxVariables(const File& fileToParse, ValueTree& elements, UndoManager* undoManager,
                                                ...
                                                float& mixingTime, float& brirSpecGain,
                                                Vector3D<float>& listenerPositionInit, Vector3D<float>& listenerPositionCurrent, Vector3D<float>& sourcePositionInit, Vector3D<float>& roomDimensions)
    {
        ...
        // the specular Gain passed on from the psx is usually not much, but it's still worth considering
        // synthesizeBRIR.m l.397
        var specGain;
        if (parsedJson.hasProperty("brirSpecGain")) {
            specGain = parsedJson.getProperty("brirSpecGain", var());
            brirSpecGain = specGain;
        }
        else
            return Result::fail("No 'revAmpMT' object found in the configuration file.");

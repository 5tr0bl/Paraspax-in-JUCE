...
#include "ParaspaxConfigurationHelper.h"
#include <thread>
#include <vector>
...
public:
    ...
    /* Micha*/
    void extrapolate();
    void initDelayProcessors(juce::dsp::ProcessSpec& spec);
    void updatePositionParameterRanges();
    AudioProcessorValueTreeState apvts;
    std::vector<DelayProcessor> delayProcessors;
    juce::dsp::ProcessSpec delayLineSpec;
    juce::dsp::ProcessSpec dfpSpec;
    DiffuseFieldProcessor dfp;
    juce::AudioBuffer<float> rirBuffer;
    std::vector<std::vector<int>> startEndArray;
    std::unique_ptr<Vector3D<float>> listenerPositionCurrent, listenerPositionInit, sourcePositionInit, sourcePositionCurrent, roomDims;
    float mixingTime;
    float revAmplitude;
    bool dfpEnabled;
    std::atomic<bool> updateDelayProcessorParams;
    bool updateRoomDims, updateXyPadPosition;
...
private:
    ...
    /* Micha */
    bool psxLoaded, idleLoadingConfig;
    const double MAX_DELAY = 2;
    Array<dsp::ProcessContextReplacing<float>> processContexts;
    //std::mutex extrapolMutex; /* Mutex for use in extrapolation function */

    /**
    *   The ratio of diff/spec is way lower in matlab
    *   Since the diffuse energy will remain constant over run-time it might be
    *   useful to apply a static attenuation factor to the DFP processing
    */
    float diffusePartScalingFactor = 0.444f;
    ...
    void timerCallback(int timerID) override
    {
        switch(timerID){
            case TIMER_PROCESSING_RELATED:
                ...
                if (updateDelayProcessorParams.exchange(false)) {
                    // Lambda function to use in thread call
                    auto extrapolateFunction = [this]() {
                        extrapolate();
                    };

                    /* update delays, amplitudes, toas, doas */
                    try {
                    ...
                    /* if statement resets the flag automatically */
                }

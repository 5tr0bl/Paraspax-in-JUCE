...
void pannerView::paint (juce::Graphics& g)
{
    ...
    if(showInputs){
        ...
        g.setOpacity(0.9f);
        for(int src=0; src<NSources; src++){
            src > 0 ? g.drawText("R" + std::to_string(src), SourceIcons[src].expanded(10.0f, 0.0f), Justification::centred, true)
                    : g.drawText("Dir", SourceIcons[src].expanded(10.0f, 0.0f), Justification::centred, true);
        }
    }

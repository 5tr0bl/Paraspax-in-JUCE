...
public:
    ...
    static void sphericalBnf2cartesian(float az, float elev, float r, float& x, float& y, float& z) {
        /*
        float azi_mapped = aziBnf2aziSph(az);
        azi_mapped = degreesToRadians(azi_mapped);
        elev = degreesToRadians(elev);
        z = r * std::sin(elev);
        auto testcos = std::cos(azi_mapped);
        auto testsin = std::sin(azi_mapped);
        float xSignum = (az > 0) ? 1 : ((az < 0) ? -1 : 0);
        double rcoselev = r * std::cos(elev);
        x = rcoselev * xSignum * std::sin(azi_mapped);//rcoselev * std::sin(azi_mapped);
        y = rcoselev * std::cos(azi_mapped);
        */
    ...
    static void cartesian2sphericalBnf(float x, float y, float z, float& az, float& elev, float& r) {
        ...
        r = std::hypot(hypotxy, z); // is this correct??
        ...
        az = aziSph2aziBnf(az);
        /*
        float altAzi = std::atan2(x, y);
        float altAziDeg = radiansToDegrees(altAzi);
        altAziDeg = aziSph2aziBnf(altAziDeg);
        */
    }

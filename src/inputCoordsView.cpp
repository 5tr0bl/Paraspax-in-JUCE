...
const int sensorEdit_width = 210;
...
inputCoordsView::inputCoordsView (PluginProcessor* ownerFilter, int _maxNCH, int _currentNCH )
{
    ...
    dummySlider->setTooltip("Here is the Dummy Slider");
    ...
    setSize (sensorEdit_width, sensorEdit_height*currentNCH); // Micha: initialize with NUM_DELAY_PROCESSORS
    hVst = ownerFilter;
    ...
    buttons = new std::unique_ptr<TextButton>[(unsigned long)maxNCH];

    int left = 22, tbw = 45, tbh = 20, tbhpad = 6; // text box layout dimensions
    int tbvpad = (sensorEdit_height - tbh) / 2;
    for( int i=0; i<maxNCH; i++){
        buttons[i] = std::make_unique<TextButton>("new tb");
        buttons[i]->setTooltip("Toggle source's on/off state.");
        buttons[i]->setBounds(0, tbvpad + i * sensorEdit_height, left+tbhpad, tbh);
        buttons[i]->setToggleState(true, dontSendNotification);
        buttons[i]->setClickingTogglesState(true);
        buttons[i]->setButtonText(i == 0 ? "D" : "R" + std::to_string(i));
        ...
        aziSliders[i]->setEnabled(false); // Micha: no drag or write operation by user allowed
        ...
        elevSliders[i]->setEnabled(false); // Micha: no drag or write operation by user allowed
        ...
        distSliders[i]->setEnabled(false); // Micha: no drag or write operation by user allowed
    }
...
void inputCoordsView::sliderValueChanged (juce::Slider* sliderThatWasMoved)
{
    ...
    //for(int i=0; i<maxNCH; i++){ --> orig. loop; delayProcessors is not as big as MAX_NUM_CHANNELS
    for(int i=0; i< currentNCH; i++){
        ...
        if (sliderThatWasMoved == distSliders[i].get()) {
            float newValue = (float)distSliders[i]->getValue();
            binauraliserNF_setSourceDist_m(hBin, i, newValue);
            hVst->delayProcessors[i].setDistance(newValue);
            break;
        }
    }
...
void inputCoordsView::buttonClicked(Button* button)
{
    for (int i = 0; i < currentNCH; i++) {
        if (button == buttons[i].get()) {
            bool newValue = (bool)buttons[i]->getToggleState();
            hVst->delayProcessors[i].setIsActive(newValue);
            break;
        }
    }
}
